...
A Valheim mod using [Jötunn](https://github.com/Valheim-Modding/Jotunn) including build tools and a basic Unity project stub. 
...
A compressed file with the binaries is created in `Qmod\Package` ready for upload to ThunderStore.
...
A Unity project is included in this repository under `QmodUnity`.
...
4. Compile the project. This copies all assemblies into `QmodUnity\Assets\Assemblies`. Don't open Unity yet before this step, it will remove assembly references.
...
6. Open Unity Hub and add the QmodUnity project
...
1. Copy all `assembly_*.dll` from `<ValheimDir>\valheim_Data\Managed` into `QmodUnity\Assets\Assemblies`. <br />
